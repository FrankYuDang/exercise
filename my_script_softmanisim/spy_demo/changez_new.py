...
if __name__ == "__main__":
    ...
    try: 
        ...
        print(f"创建形状出错: {e}"); p.disconnect(physicsClientId); sys.exit(1)
    ...
    try:
        while simulation_running:
            ...
            if current_row_index >= num_data_rows:
                if play_mode == 'stop_after_one_run': 
                    current_row_index = 0
                else: 
                    print("[信息] 所有数据已播放完毕。")
                    ...
                    try:
                        ...
                        if len(my_robot_bodies) >= num_tip_bodies:
                         ...
                         if frame_count % 120 == 0: 
                             print(f"警告: 更新末端或记录位置时出错 - {e}")
                         pos_tip_world_m = np.array([np.nan, np.nan, np.nan]) # 出错时记录 NaN
            elif sol is None or sol.shape[1] < 3:
                 if frame_count % 120 == 0: 
                     print(f"警告: 第 {current_row_index} 帧无法获取有效形状({sol.shape if sol is not None else 'None'})。")
