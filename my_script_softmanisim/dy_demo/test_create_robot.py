# 将该文档放在 scripts文件夹下运行
# 添加测试断点，观察代码运行，重点观察 positions, sol, my_soft_bodies 的变化 
# update:
# 2025-04-15 - 增加代码来源的注释，复习流程


import sys
import os

# 如果需要独立运行文档的时候 采用下面的代码
# # 获取 SoftManiSim 文件夹的绝对路径
# softmanisim_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))

# # 将 SoftManiSim 文件夹添加到 sys.path
# softmanisim_path = "C:\Document\GITHUB\SoftManiSim"
...
if __name__ == "__main__":
    ...
    #! Below code is based on the .\pybullet_env\BasicEnvironment.py
    #! Class SoftRobotBasicEnvironment.__init__()
    ...
    #! Based on visualizer.visualizer ODE() 
    my_ode = ODE()
    #! SoftRobotBasicEnvironment.create_robot()
    act = np.array([0,0,0])
    ...
    # load the position of each sphere 
    ...
    while True:
        ...
        #! Pybullet_env.BasicEnvironment.move_robot_ori()
        # action 里面的元素 
